...
const Register = () => {
  ...
  const [isSendingOtp, setIsSendingOtp] = useState(false); // New state for send OTP loader
  ...
  const handleSendOtp = async () => {
    setIsSendingOtp(true); // Start the loader
    try {
      const email = getValues('email');
      if (!email) {
        toast.error("Please enter a valid email address first.", { autoClose: 3000 });
        return;
      }
      ...
      setIsSendingOtp(false); // Stop the loader
    }
  };
  ...
  const handleVerifyOtp = async () => {
    ...
    try {
      ...
      toast.error(error.response?.data?.message || "OTP verification failed. Please check the code.", {
        autoClose: 3000,
      });
  ...
  const onSubmit = async (data) => {
    if (!isOtpVerified) {
      toast.error("Please verify your email address first.", { autoClose: 3000 });
      return;
    }
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
            ...
            {isOtpSent && (
              <div className="space-y-4">
                <label className="block text-xl font-medium text-[#113A5F] mb-2">Verify OTP</label>
                <div className="flex gap-2">
                  <input
                    type="text"
                    value={otp}
            ...
            <button
              type="submit"
              className={`w-full py-3 text-white text-2xl rounded-lg font-semibold transition ${isOtpVerified ? 'bg-[#53974A] hover:bg-green-700' : 'bg-gray-400 cursor-not-allowed'}`}
