...
const createBooking = async (req, res) => {
  ...
  try {
    ...
    if (!customer.address) {
      return res
        .status(400)
        .json({ error: "All Address details are required." });
    }
    if (!payment) {
    ...
    for (const traveler of travelers) {
      if (
        !traveler.name ||
        typeof traveler.age === "undefined" ||
        !traveler.gender
      ) {
        return res
          .status(400)
          .json({ error: "All travelers must have a name, age, and gender." });
      }
    }
    ...
    if (!existingCustomer) {
      const namePart = (customer.name || "cust")
        .substring(0, 4)
        .padEnd(4, "x")
        .toLowerCase();
      const dob = customer.dob || "2000-01-01";
      const dobMonthDay = new Date(dob)
        .toISOString()
        .slice(5, 10)
        .replace("-", "");
    ...
    return res
      .status(500)
      .json({
        error: "Failed to create or update booking",
        details: error.message,
      });
  }
};
    ...
    if (tourID) {
      ...
      if (mongoose.Types.ObjectId.isValid(tourID)) {
        ...
        return res
          ...
          .json({
            message:
              "Invalid Tour ID format. Must be a valid MongoDB ObjectId.",
          });
      }
    }
    if (tourName) {
...
router.get("/my-bookings", authenticate, async (req, res) => {
  try {
    ...
    console.log("Searching for bookings for user ID:", userId);

    const bookings = await Booking.find({ "customer.id": userId });

    console.log("Bookings found in DB:", bookings);
