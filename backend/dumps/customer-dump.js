...
router.get('/daily-dump', async (req, res) => {
  try {
    ...
    // ✅ Example daily range — can be adjusted dynamically if needed
    ...
    res.setHeader('Content-Disposition', `attachment; filename=daily_dump_${today.toISOString().slice(0, 10)}.xlsx`);
...
// ===============================
// ✅ WEEKLY DUMP (same logic, different date range)
// ===============================
router.get('/weekly-dump', async (req, res) => {
  ...
  req.originalUrl = '/weekly-dump';
  return generateDump(req, res, startOfWeek, today, 'Weekly Booking Dump', 'weekly_dump');
});


// ===============================
// ✅ QUARTERLY DUMP (same logic, 3-month range)
// ===============================
router.get('/quarterly-dump', async (req, res) => {
  ...
  req.originalUrl = '/quarterly-dump';
  return generateDump(req, res, startOfQuarter, today, 'Quarterly Booking Dump', 'quarterly_dump');
});
